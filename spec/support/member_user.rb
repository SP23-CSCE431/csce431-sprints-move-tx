...
before do
  ...
  current_admin = Admin.create!(email: 'waylandmoody55@tamu.edu', full_name: 'Jerry Moody', id: 1)
  # stub current_admin method
  allow(view).to receive(:current_admin).and_return(current_admin)
  # create a member with the admin as its foreign key
  user = Member.create!(position: 'Member', id: 1, name: 'wayland', admin_id: 1)
  # assign the member to @member
  assign(:member, user)
  assign(:user, user)
end
end
